...
| `chrome?` | `WidgetChromeContribution \| undefined` |  |
...
```ts
type WidgetModalSize = "sm" | "md" | "content" | "lg" | "xl"
```
...
| `homepage?` | `string \| undefined` | Product homepage used for service-owned brand metadata such as favicons. |
...
```ts
type WidgetChromeHook = (...args: any[]) => unknown
```
...
| Property | Type | Description |
|----------|------|-------------|
| `hooks?` | `Record<string, WidgetChromeHook> \| undefined` |  |
...
| `supportedDashboardScopes?` | `("all-projects" \| "project")[] \| undefined` | Dashboard scopes where this widget can be placed. Omit to support all scopes. |
...
| `chrome?` | `WidgetChromeContribution \| undefined` | Optional app chrome data hooks exposed through the widget registry. |
