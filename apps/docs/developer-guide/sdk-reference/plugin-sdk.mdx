...
| `size?` | `ModalSize \| undefined` | Size hint for modal / side-panel presentations. |
...
| `presentationSize?` | `ModalSize \| undefined` | Size hint for modal / side-panel presentations (legacy — prefer `PluginPresentationConfig.size`). |
...
| `server?` | `PluginServerDefinition \| undefined` | Optional server-side hooks and route handlers owned by this plugin. App routes look these up through PLUGIN_REGISTRY instead of importing plugin internals directly. |
...
```ts
type PluginServerRouteHandler = (input: PluginServerRouteInput) => Promise<PluginServerRouteResult>
```
...
| Property | Type | Description |
|----------|------|-------------|
| `configure?` | `((runtime: PluginServerRuntime) => void) \| undefined` | Called once by the host app to pass server-only services to the plugin. |
| `routes?` | `Record<string, PluginServerRouteHandler> \| undefined` | Named route handlers delegated to by app shell routes. |
