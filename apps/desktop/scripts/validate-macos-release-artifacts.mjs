...
const SIDECAR_PROBE_TIMEOUT_MS = 10_000;
const SIDECAR_PROBE_PATH = "/api/notifications";
...
function probeHttpUrl(url, timeoutMs = SIDECAR_PROBE_TIMEOUT_MS) {
  ...
  await new Promise((resolvePromise, rejectPromise) => {
    ...
    child.stdout.on("data", (chunk) => {
      ...
      if (url && !probing) {
        ...
        probeHttpUrl(probeUrl.href, probeTimeoutMs)
          .then(() => settle())
          .catch((error) => {
            settle(
              new Error(
                `Sidecar runtime printed ${url} but ${probeUrl.pathname} did not respond successfully: ${error instanceof Error ? error.message : String(error)}.${runtimeOutput()}`
              )
            );
          });
      }
...
async function validateApp(appPath, options) {
  ...
  const mainBinaryPath = join(resolvedAppPath, "Contents", "MacOS", "Radarboard");
