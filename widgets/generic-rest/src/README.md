...
> Scaffolded with `pnpm create-widget generic-rest`

## Quick Start

1. **Define the recipe** — Edit the template recipe in `index.ts` to set your layout (list, chart, summary, etc.)
2. **Wire data** — Implement the data-fetching hook in `hooks/use-generic-rest.ts`
3. **Register the data source** — Connect hook data in `data-resolver.tsx`
4. **Test** — Run `pnpm --filter @radarboard/widget-generic-rest test`
...
├── components/
│   ├── generic-rest-compact.tsx   # Grid card view (3x3 grid)
│   └── generic-rest-expanded.tsx  # Expanded overlay view
├── hooks/
│   └── use-generic-rest.ts        # SWR data-fetching hook
├── mcp/
...
1. **Hook** (`use-generic-rest.ts`) — Fetches from `/api/integrations/generic-rest/data`
