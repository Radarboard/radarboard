...
- [ ] Re-authenticate `gh` or install the GitHub app for the `Radarboard` org.
- [ ] Create empty org repositories with no generated README, license, or gitignore.
- [ ] Seed each org repo from a clean snapshot with a single `chore: initialize Radarboard` commit.
- [ ] Configure repo settings, rulesets, variables, secrets, and external integrations.
...
Local source references have been moved to the new organization targets. The docs build now validates under the current Mintlify CLI. Local lint, typecheck, test, and build gates pass. Remote repository creation and seeding remain blocked until GitHub auth or GitHub App access includes the `Radarboard` organization.
