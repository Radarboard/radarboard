...
const railListExample = COMPOSITION_EXAMPLES.find((example) => example.recipeKind === "rail_list");

if (!railListExample) {
  throw new Error("Missing canonical composition example for rail_list");
}

const railListConfig = synchronizeTemplateConfig(railListExample.config);

const meta = preview.meta({
  title: "Widgets/Recipe Examples/Rail + List",
  parameters: {
    layout: "fullscreen",
  },
  args: {
  ...
  render: ({ mode }) => (
    <div className="h-[360px] overflow-hidden border border-border bg-surface p-3">
      <div className="h-full overflow-hidden border border-border bg-surface-raised">
        {mode === "expanded" ? (
          <TemplateWidgetExpanded
            widgetId="story:rail-list"
            projectSlug={null}
            config={railListConfig}
          />
        ) : (
          <TemplateWidget widgetId="story:rail-list" projectSlug={null} config={railListConfig} />
        )}
      </div>
    </div>
  ),
