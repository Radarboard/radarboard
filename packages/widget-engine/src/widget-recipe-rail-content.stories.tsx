...
const railContentExample = COMPOSITION_EXAMPLES.find(
  (example) => example.recipeKind === "rail_content"
);

if (!railContentExample) {
  throw new Error("Missing canonical composition example for rail_content");
}

const railContentConfig = synchronizeTemplateConfig(railContentExample.config);

const meta = preview.meta({
  title: "Widgets/Recipe Examples/Rail + Content",
  parameters: {
    layout: "fullscreen",
  },
  args: {
  ...
  render: ({ mode }) => (
    <div className="h-[360px] overflow-hidden border border-border bg-surface p-3">
      <div className="h-full overflow-hidden border border-border bg-surface-raised">
        {mode === "expanded" ? (
          <TemplateWidgetExpanded
            widgetId="story:rail-content"
            projectSlug={null}
            config={railContentConfig}
          />
        ) : (
          <TemplateWidget
            widgetId="story:rail-content"
