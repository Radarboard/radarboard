...
**Source:** apps/app/content/patterns/forms/button.mdx
...
- **Responsive considerations**: When button text is hidden on mobile (icon-only), preserve context through:
  - Keep the icon visible with an inline spinner overlay
  - Use `aria-live` regions to announce status changes
  - Consider tooltips or micro-animations to indicate what's loading
  - Maintain `aria-label` that includes the action being performed
...
10. **Loading States**
    ...
    ```html
    <!-- Standard loading with text -->
    <button type="button" aria-busy="true">
      <svg class="spinner" aria-hidden="true">...</svg>
      <span>Loading...</span>
    </button>

    <!-- Responsive: Icon-only on mobile with preserved context -->
    <button type="button" aria-busy="true" aria-label="Save document">
      <svg class="icon-save" aria-hidden="true">...</svg>
      <span class="text-desktop-only">Save</span>
      <svg class="spinner-overlay" aria-hidden="true">...</svg>
...
- When buttons become icon-only on mobile, maintain context during loading with `aria-live` announcements
- Preserve the original icon during loading states for icon-only buttons
- Use `aria-busy="true"` during async operations to inform assistive technologies
...
- Use appropriate color contrast ratios (WCAG 2.2 AA compliance)
...
- Don't replace icon-only buttons entirely with spinners—keep the icon visible for context
- Don't assume loading indicators alone convey what's happening on mobile devices
...
- For icon-only buttons on mobile, keep the icon visible during loading with inline spinners
- Provide status feedback through subtle animations or micro-interactions when text is hidden
- Consider using `aria-live` regions for important state changes in icon-only buttons
...
- [ ] Maintain sufficient color contrast ratios (WCAG 2.2 AA)
