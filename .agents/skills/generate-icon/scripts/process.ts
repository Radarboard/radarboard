...
/**
 * Fix AI-generated images that have baked-in rounded corners.
 * Strategy: sample the background color from edge midpoints, create a solid
 * background of that color, then scale the original image up ~8% and composite
 * it centered — the slight zoom pushes the rounded corners off-canvas.
 */
export async function fillCorners(
  inputPath: string,
  outputPath: string
): Promise<void> {
  ...
  // Sample edge midpoint pixels to determine background color
  ...
  // Sample from ~15% inward along each edge midpoint (inside the rounded corner radius)
  const inset = Math.floor(Math.min(width, height) * 0.15);
  const samplePoints = [
    px(inset, 0), px(width - inset - 1, 0),                   // top edge
    px(inset, height - 1), px(width - inset - 1, height - 1), // bottom edge
    px(0, inset), px(0, height - inset - 1),                   // left edge
    px(width - 1, inset), px(width - 1, height - inset - 1),  // right edge
  ].filter(p => p.a > 200);
  ...
  const bgR = Math.round(samplePoints.reduce((s, p) => s + p.r, 0) / samplePoints.length);
  const bgG = Math.round(samplePoints.reduce((s, p) => s + p.g, 0) / samplePoints.length);
  const bgB = Math.round(samplePoints.reduce((s, p) => s + p.b, 0) / samplePoints.length);

  console.log(`  Sampled edge background color: rgb(${bgR}, ${bgG}, ${bgB})`);

  // Scale original up by 8% so the rounded corners get pushed off the canvas
  const scale = 1.08;
  const scaledSize = Math.round(Math.max(width, height) * scale);
  const scaledImage = await sharp(inputPath)
    .resize(scaledSize, scaledSize, { kernel: sharp.kernel.lanczos3, fit: "cover" })
    .toBuffer();

  // Crop back to original size (center crop)
  const offsetX = Math.floor((scaledSize - width) / 2);
  const offsetY = Math.floor((scaledSize - height) / 2);
  const croppedImage = await sharp(scaledImage)
    .extract({ left: offsetX, top: offsetY, width, height })
    .toBuffer();

  // Composite onto solid background to fill any remaining edge artifacts
  await sharp({
    create: {
      width,
      height,
