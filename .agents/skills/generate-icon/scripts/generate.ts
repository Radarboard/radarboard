...
// Icon-optimized prompt template (transparent background, glyph-style)
...
// Apple-style icon prompt: full-bleed square with layered depth
// IMPORTANT: Avoid saying "app icon" — image models render rounded corners when they hear that.
const APPLE_ICON_PROMPT_TEMPLATE = `Create a digital illustration for: {userPrompt}

CRITICAL COMPOSITION REQUIREMENTS:
- The image must be a PERFECT SQUARE with SHARP 90-degree corners — absolutely NO rounded corners, NO border radius, NO soft edges
- The background gradient or color must extend to ALL FOUR EDGES and into every corner pixel — zero margin, zero padding, zero border
- Rich layered design with DEPTH: a vibrant background gradient, a prominent centered symbol, and subtle lighting/shadow
- The foreground symbol should have depth: subtle drop shadow, inner highlights, or 3D feel
- Polished, layered, dimensional — like a premium software product illustration
- NO transparency, NO cutouts — every pixel of the square must be filled with color
- NO text, NO words, NO letters
- NO rounded rectangle shape, NO icon frame, NO bezel — just the raw artwork edge-to-edge
- Single focal point, clean vector-like rendering`;
...
async function main() {
  ...
  if (args.masterImage) {
    ...
    const isApplePreset = args.preset === "macos-icns" || args.preset === "apple-app-icon";
    const template = isApplePreset ? APPLE_ICON_PROMPT_TEMPLATE : ICON_PROMPT_TEMPLATE;
    const basePrompt = template.replace("{userPrompt}", args.prompt);
  ...
  // Stage 1.5: Fix rounded corners for Apple presets (AI models bake in rounded corners)
  let currentMaster = masterRawPath;
  const isApplePreset = args.preset === "macos-icns" || args.preset === "apple-app-icon";
  if (isApplePreset) {
    console.log("\nStage 1.5: Fixing AI-baked rounded corners...");
    const fixedPath = join(args.output, "master-squared.png");
    await fillCorners(currentMaster, fixedPath);
    currentMaster = fixedPath;
    console.log("  Saved: master-squared.png");
  }

  // Stage 2: Remove background (skip for Apple presets — they need full-bleed square backgrounds)
  if (!args.skipRemoveBg && !isApplePreset) {
    ...
    const reason = isApplePreset ? "Apple preset — keeping full-bleed background" : "--skip-remove-bg";
    console.log("\nStage 2: Skipped (" + reason + ")");
  }

  // Stage 3: Crop and center (skip for Apple presets — full-bleed square, just resize to master size)
  if (isApplePreset) {
    console.log("\nStage 3: Resizing to master size (Apple preset — preserving full-bleed design)...");
    await sharp(currentMaster)
      .resize(preset.masterSize, preset.masterSize, {
        kernel: sharp.kernel.lanczos3,
        fit: "cover",
      })
      .png({ compressionLevel: 9 })
      .toFile(masterCroppedPath);
  ...
  // Stage 3.5: Fill background for non-transparent presets that aren't Apple
  // (Apple presets already have a full-bleed background from generation)
  if (!preset.requiresTransparency && !isApplePreset) {
    console.log("\nStage 3.5: Filling background...");
    ...
    console.log(`  Saved: master-final.png`);
  }
